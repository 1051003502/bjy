...
void DRW_Header::parseCode(int code, dxfReader* reader)
{
	switch (code)
	{
		case 9:
			...
			if (version < DRW::R2000 && name == "$DIMUNIT")
				name = "$DIMLUNIT";
			vars[name] = curr;
			break;
		case 1:
			...
			if (name == "$ACADVER")
			{
				reader->setVersion(curr->content.pStr, true);
		...
		case 3:
			...
			if (name == "$DWGCODEPAGE")
			{
				reader->setCodePage(curr->content.pStr);
...
{
	/*RLZ: TODO complete all vars to R2010*/
	double varDouble;
	int varInt;
	std::string varStr;
	DRW_Coord varCoord;
	...
	switch (ver)
	{
		...
		case DRW::AC1009: //acad 11 & 12
			varStr = "AC1009";
			break;
		case DRW::R13: //unsupported version acad 13
		case DRW::R14: //acad 14
			varStr = "AC1014";
			break;
		case DRW::R2000: //acad 2000
			varStr = "AC1015";
			break;
		case DRW::R2004: //acad 2004
			varStr = "AC1018";
			break;
			/*    case DRW::R2007: //acad 2007
					varStr = "AC1021";
					break;*/
		case DRW::R2010: //acad 2010
			varStr = "AC1024";
			break;
		case DRW::R2013: //acad 2013
			varStr = "AC1027";
			break;
...
bool DRW_Header::parseDwg(DRW::Version version, dwgBuffer* buf, dwgBuffer* hBbuf, duint8 mv)
{
	...
	if ((version > DRW::R2007 && mv > 3) || version == DRW::R2018)
	{ //2010+
	...
	// !msclock pending to verify and check
	// DRW_DBG("\nUnknown text1: "); DRW_DBG(buf->getVariableText(version, false));
	// DRW_DBG("\nUnknown text2: "); DRW_DBG(buf->getVariableText(version, false));
	// DRW_DBG("\nUnknown text3: "); DRW_DBG(buf->getVariableText(version, false));
	// DRW_DBG("\nUnknown text4: "); DRW_DBG(buf->getVariableText(version, false));
...
//    vars["TDUSRTIMER"]=new DRW_Variant(40, buf->getBitLong());//RLZ: TODO convert to day.msec
	...
	control = hBbuf->getHandle();
	DRW_DBG("\nBLOCK_RECORD_PAPER_SPACE: "); DRW_DBGHL(control.code, control.size, control.ref);
	control = hBbuf->getHandle();
	DRW_DBG("\nBLOCK_RECORD_MODEL_SPACE: "); DRW_DBGHL(control.code, control.size, control.ref);
	control = hBbuf->getHandle();
	DRW_DBG("\nLTYPE_BYLAYER: "); DRW_DBGHL(control.code, control.size, control.ref);
	control = hBbuf->getHandle();
	DRW_DBG("\nLTYPE_BYBLOCK: "); DRW_DBGHL(control.code, control.size, control.ref);
	control = hBbuf->getHandle();
		...
		for (auto it = vars.begin(); it != vars.end(); ++it)
		{
			...
			switch (it->second->type())
			{
				...
				case DRW_Variant::STRING:
					DRW_DBG(it->second->content.pStr->c_str());
					break;
				case DRW_Variant::COORD:
					DRW_DBG("x= "); DRW_DBG(it->second->content.pCrd->x);
					DRW_DBG(", y= "); DRW_DBG(it->second->content.pCrd->y);
					DRW_DBG(", z= "); DRW_DBG(it->second->content.pCrd->z);
					break;
				default:
					break;
			}
	...
	buf->setPosition((duint64)size + 16 + 4); //readed size +16 start sentinel + 4 size
	if ((version > DRW::R2007 && mv > 3) || version == DRW::R2018)
	{ //2010+
		buf->getRawLong32();//advance 4 bytes (hisize)
	}
	DRW_DBG("\ncurrent position to: "); DRW_DBG(buf->getPosition());
	DRW_DBG("\nHeader CRC: "); DRW_DBGH(buf->getRawShort16());
	DRW_DBG("\nbuf position: "); DRW_DBG(buf->getPosition());
	DRW_DBG("\nread dwg header end sentinel= ");
	...
	DRW_DBG("\n");
#endif
	DRW::checkSentinel(buf,SecEnum::DWGSection::HEADER,false);
