...
bool dxfRW::writeEntity(DRW_Entity* ent)
{
	...
	if (version > DRW::R2000 && ent->color24 >= 0)
	{
...
{
	std::string strname = ent->entryName;

	transform(strname.begin(), strname.end(), strname.begin(), ::toupper);
	//do not write line types handled by library
	...
	if (version > DRW::AC1009)
	{
		...
		writer->writeUtf8String(2, ent->entryName);
	}
	else
		writer->writeUtf8Caps(2, ent->entryName);
	writer->writeInt16(70, ent->flags);
	writer->writeUtf8String(3, ent->description);
	ent->updateTotalLengthOfPattern();
	writer->writeInt16(72, 65);
	writer->writeInt16(73, ent->dashesNum);
	writer->writeDouble(40, ent->patternLen);

	for (unsigned int i = 0; i < ent->dashes.size(); i++)
	{
		writer->writeDouble(49, ent->dashes.at(i).dashLength);
		if (version > DRW::AC1009)
		{
...
bool dxfRW::writeVport(DRW_Vport* ent)
{
	...
	writer->writeDouble(12, ent->viewCenter.x);
	writer->writeDouble(22, ent->viewCenter.y);
	...
	writer->writeDouble(40, ent->viewHeight);
	writer->writeDouble(41, ent->aspectRatio);
	...
	writer->writeDouble(51, ent->viewTwistAngle);
...
bool dxfRW::writeDimstyle(DRW_Dimstyle* ent)
{
	...
	if (version > DRW::R2004 || ent->dimfxl != 0)
	...
	if (version > DRW::AC1009)
	{
		...
		writer->writeInt16(286, ent->dimalttz);
		if (version < DRW::R2000)
	...
	if (version > DRW::R2004 && ent->dimfxlon != 0)
...
bool dxfRW::writeArc(DRW_Arc* ent)
{
	...
	writer->writeDouble(50, ent->sAngle * ARAD);
	writer->writeDouble(51, ent->eAngle * ARAD);
	return true;
}
...
bool dxfRW::writeEllipse(DRW_Ellipse* ent)
{
	...
	if (version > DRW::AC1009)
	{
		...
		writer->writeDouble(21, ent->secPoint.y);
		writer->writeDouble(31, ent->secPoint.z);
		writer->writeDouble(40, ent->minormajoratio);
		writer->writeDouble(41, ent->sAngle);
		writer->writeDouble(42, ent->eAngle);
	}
	else
	{
		DRW_Polyline pol;
...
bool dxfRW::writePolyline(DRW_Polyline* ent)
{
	...
	writer->writeInt16(70, ent->flags);
	if (ent->defStartWidth != 0)
	{
		writer->writeDouble(40, ent->defStartWidth);
	}
	...
	{
		writer->writeInt16(71, ent->vertCount);
		writer->writeInt16(72, ent->faceCount);
	}
	if (ent->smoothMDensity != 0)
	{
		writer->writeInt16(73, ent->smoothMDensity);
	}
	if (ent->smoothNDensity != 0)
	{
		writer->writeInt16(74, ent->smoothNDensity);
	}
	if (ent->curvetype != 0)
	{
	...
	int vertexnum = ent->verts.size();
	for (int i = 0; i < vertexnum; i++)
	{
		DRW_Vertex* v = ent->verts.at(i);
...
bool dxfRW::writeSpline(DRW_Spline* ent)
{
	if (version > DRW::AC1009)
	{
		...
		writer->writeInt16(72, ent->knotsNum);
		writer->writeInt16(73, ent->ctlPtsNum);
		writer->writeInt16(74, ent->fitPtsNum);
		...
		for (int i = 0; i < ent->knotsNum; i++)
		{
			writer->writeDouble(40, ent->knotslist.at(i));
		}
		for (int i = 0; i < ent->ctlPtsNum; i++)
		{
...
bool dxfRW::writeHatch(DRW_Hatch* ent)
{
	if (version > DRW::AC1009)
	{
		...
		writer->writeInt16(70, ent->solidFill);
		writer->writeInt16(71, ent->associative);
		ent->numPaths = ent->paths.size();
		writer->writeInt16(91, ent->numPaths);
		//write paths data
		for (int i = 0; i < ent->numPaths; i++)
		{
			auto loop = ent->paths.at(i);
			writer->writeInt16(92, loop->pathFlag);
			if ((loop->pathFlag & 2) == 2)
			{
			...
			else
			{
				//boundary path
				loop->updateEdgesNum();
				writer->writeInt16(93, loop->edgesNum);
				for (int j = 0; j < loop->edgesNum; ++j)
				{
					const auto& entInLoop = loop->entities.at(j);
					switch (entInLoop->dwgType)
					{
						case DRW::LINE:
						{
							writer->writeInt16(72, 1);
							auto l = std::dynamic_pointer_cast<DRW_Line>(loop->entities.at(j));
						...
						case DRW::ARC:
						{
							...
							auto a = std::dynamic_pointer_cast<DRW_Arc>(loop->entities.at(j));
							writer->writeDouble(10, a->basePoint.x);
							writer->writeDouble(20, a->basePoint.y);
							writer->writeDouble(40, a->radious);
							writer->writeDouble(50, a->sAngle * ARAD);
							writer->writeDouble(51, a->eAngle * ARAD);
						...
						case DRW::ELLIPSE:
						{
							...
							auto a = std::dynamic_pointer_cast<DRW_Ellipse>(loop->entities.at(j));
							a->correctAxis();
							writer->writeDouble(10, a->basePoint.x);
							writer->writeDouble(20, a->basePoint.y);
							writer->writeDouble(11, a->secPoint.x);
							writer->writeDouble(21, a->secPoint.y);
							writer->writeDouble(40, a->minormajoratio);
							writer->writeDouble(50, a->sAngle * ARAD);
							writer->writeDouble(51, a->eAngle * ARAD);
							writer->writeInt16(73, a->isccw);
							break;
	...
	//                        writer->writeInt16(72, 4);
		...
		writer->writeInt16(75, ent->hatchStyle);
		writer->writeInt16(76, ent->hatchPatternType);
		if (!ent->solidFill)
		{
			writer->writeDouble(52, ent->hatchAngle);
			writer->writeDouble(41, ent->scaleOrSpacing);
			writer->writeInt16(77, ent->doubleHatchFlag);
...
bool dxfRW::writeDimension(DRW_Dimension* ent)
{
	if (version > DRW::AC1009)
	{
		...
		writer->writeDouble(10, ent->get10pt().x);
		writer->writeDouble(20, ent->get10pt().y);
		writer->writeDouble(30, ent->get10pt().z);
		...
		switch (ent->dwgType)
		{
			...
			case DRW::DIMENSION_LINEAR:
			{
				...
				if (ent->dwgType == DRW::DIMENSION_LINEAR)
				{
			...
			{
				DRW_DimRadius* dd = (DRW_DimRadius*)ent;
			...
			{
				DRW_DimDiameter* dd = (DRW_DimDiameter*)ent;
...
bool dxfRW::writeInsert(DRW_Insert* ent)
{
	...
	if (version > DRW::AC1009)
	{
		...
		writer->writeUtf8String(2, ent->blockName);
	}
	else
		writer->writeUtf8Caps(2, ent->blockName);
	writer->writeDouble(10, ent->basePoint.x);
	writer->writeDouble(20, ent->basePoint.y);
	writer->writeDouble(30, ent->basePoint.z);
...
bool dxfRW::writeText(DRW_Text* ent)
{
	...
	writer->writeDouble(41, ent->rectWidth);
...
DRW_ImageDef* dxfRW::writeImage(DRW_Image* ent, std::string name)
{
	if (version > DRW::AC1009)
	{
		...
		{
			if (imageDef.at(i)->entryName == name)
			{
				id = imageDef.at(i);
				continue;
			}
...
bool dxfRW::writeObjects()
{
	...
	if (imageDef.size() != 0)
	{
		...
		{
			size_t f1, f2;
			f1 = imageDef.at(i)->entryName.find_last_of("/\\");
			f2 = imageDef.at(i)->entryName.find_last_of('.');
			++f1;
			writer->writeString(3, imageDef.at(i)->entryName.substr(f1, f2 - f1));
	...
	for (unsigned int i = 0; i < imageDef.size(); i++)
	{
		...
		writer->writeUtf8String(1, id->entryName);
		...
		switch ((*it)->code())
		{
			...
			case 1005:
			{
				...
				if ((*it)->type() == DRW_Variant::STRING)
					writer->writeUtf8String(cc, *(*it)->content.pStr);
			...
			case 1013:
				...
				{
					writer->writeDouble((*it)->code(), (*it)->content.pCrd->x);
					writer->writeDouble((*it)->code() + 10, (*it)->content.pCrd->y);
					writer->writeDouble((*it)->code() + 20, (*it)->content.pCrd->z);
				}
				break;
			case 1040:
			case 1041:
...
bool dxfRW::processLType()
{
	...
	std::string sectionstr;
	bool reading = false;
	auto ltype = std::make_shared<DRW_LType>();
	while (reader->readRec(&code))
	{
		DRW_DBG(code); DRW_DBG("\n");
		if (code == 0)
		{
			if (reading)
			{
				ltype->updateTotalLengthOfPattern();
...
bool dxfRW::processLayer()
{
	...
	std::string sectionstr;
	bool reading = false;
	auto layer = std::make_shared<DRW_Layer>();
	while (reader->readRec(&code))
	{
		DRW_DBG(code); DRW_DBG("\n");
		if (code == 0)
...
bool dxfRW::processDimStyle()
{
	...
	std::string sectionstr;
	bool reading = false;
	auto dimSty = std::make_shared<DRW_Dimstyle>();
	while (reader->readRec(&code))
	{
		DRW_DBG(code); DRW_DBG("\n");
		if (code == 0)
...
bool dxfRW::processTextStyle()
{
	...
	std::string sectionstr;
	bool reading = false;
	auto TxtSty = std::make_shared<DRW_Textstyle>();
	while (reader->readRec(&code))
	{
		DRW_DBG(code); DRW_DBG("\n");
		if (code == 0)
...
bool dxfRW::processVports()
{
	...
	std::string sectionstr;
	bool reading = false;
	auto vp = std::make_shared<DRW_Vport>();
	while (reader->readRec(&code))
	{
		DRW_DBG(code); DRW_DBG("\n");
		if (code == 0)
...
bool dxfRW::processAppId()
{
	...
	std::string sectionstr;
	bool reading = false;
	auto vp = std::make_shared<DRW_AppId>();
	while (reader->readRec(&code))
	{
		DRW_DBG(code); DRW_DBG("\n");
		if (code == 0)
		...
		switch (code)
		{
			case 0:
			{
				...
				switch (type)
				{
					...
					case 3:
					{
						DRW_DimDiameter d(dim);
						iface->addDimDiameter(&d);
						break;
					}
					case 4:
					{
						DRW_DimRadius d(dim);
						iface->addDimRadius(&d);
						break;
					}
					case 5:
					{
