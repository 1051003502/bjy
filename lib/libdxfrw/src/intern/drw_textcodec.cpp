...
void DRW_TextCodec::setVersion(int v, bool dxfFormat) {
	...
	else if (v == DRW::R13 || v == DRW::R14
			 || v == DRW::R2000 || v == DRW::R2004)
	{
		// R2004 and before
		version = DRW::R2000;
...
void DRW_TextCodec::setVersion(std::string* v, bool dxfFormat) {
	...
	{
		setVersion(DRW::R2000, dxfFormat);
	}
	setVersion(DRW::R2007, dxfFormat);
}
...
void DRW_TextCodec::setCodePage(std::string* c, bool dxfFormat) {
	...
	delete conv;
	if (version == DRW::AC1009 || version == DRW::R2000)
	{
		if (cp == "ANSI_874")
		...
		{
			cp = "GB2312";
			conv = new DRW_ExtConverter(cp.c_str());
		}
	}
	else
	{
		//if (min_ver <= DRW::R2004)
		//{
		//	conv = new DRW_ExtConverter("SJIS");
		//}
		//else
...
std::string DRW_ExtConverter::convertByiconv(const char* in_encode,
	...
	int err = errno;
	if (ic < 0) { printf("errno=%d\n", err); abort(); }
	size_t il = s->size(), ol = out.size();
#if _WIN32
	iconv(ic, (char**)&pIn, &il, &pOut, &ol);
#elif __linux__
	iconv(ic, (char**)&pIn, &il, &pOut, &ol);
#else
