...
{  //H
	...
	hl.ref = 0;
#if 1
	std::vector<duint8> check;
	check.resize(hl.size);
	for (int i = 0; i < hl.size; i++)
	{
		check[i] = getRawChar8();
		hl.ref = ((hl.ref << 8) | check[i]);
	}
#endif
#if 0
...
dwgHandle dwgBuffer::getOffsetHandle(duint32 href)
{  //H
...
{
	duint16 length = getBitShort();
	if (length == 0)
		return std::string();
	//duint8* tmpBuffer = new duint8[length];
	std::vector<duint8> tmpBuffer(length + 1);
	bool good = getBytes(tmpBuffer.data(), length);
	if (!good)
		return std::string();
	...
	/*    filestr->read (buffer,textSize);
		...
		//todo check its correction since remove its tail \0 seq
	std::string str(reinterpret_cast<char*>(tmpBuffer.data()), length - 1);
	return str;
}
...
std::string dwgBuffer::get16bitStr(duint16 textSize, bool nullTerm)
{
	...
	textSize = textSize + 2;  // padding to null term
...
UTF8STRING dwgBuffer::getCP8Text()
{
	std::string str;
	str = get8bitStr();  //RLZ correct these function
	if (decoder == nullptr)
		return str;

	return decoder->toUtf8(str);
}
...
//TU unicode 16 bit (UCS) text converted to utf8
//nullTerm = true if string are 2 bytes null terminated from the stream
UTF8STRING dwgBuffer::getUCSText(bool nullTerm)
{
	std::string strData;
	...
	std::string utf8Str = decoder->toUtf8(strData);
	return utf8Str;
}

//RLZ: read a T or TU if version is 2007+
//nullTerm = true if string are 2 bytes null terminated from the stream
UTF8STRING dwgBuffer::getVariableText(DRW::Version v, bool nullTerm)
{  //TV
	if (v > DRW::R2004)
...
duint16 dwgBuffer::getObjType(DRW::Version v)
{  //OT
	if (v > DRW::R2007)
	{
		duint8 b = get2Bits();
...
duint32 dwgBuffer::getCmColor(DRW::Version v)
{
	if (v < DRW::R2004)  //2000-
		return getSBitShort();
	duint16 colorIndex = getBitShort();
	duint32 rgb = getBitLong();
	duint8 colorByte = getRawChar8();
	duint8 colorType = rgb >> 24;
...
duint32 dwgBuffer::getEnColor(DRW::Version v)
{
	...
	duint16 colorIndex = 0;
	duint16 colorFlags = 0;
	duint16 colorNumber = getBitShort();
	...
	return colorNumber;  //default return ByLayer
}
...
bool dwgBuffer::isGood()
{
	return filestr->good();
}

/* reads "size" bytes and stores in "buf" return false if fail */
bool dwgBuffer::getBytes(duint8* buf, duint64 size)
{
	duint8 tmp;
