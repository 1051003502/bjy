...
void DwgReader18::parseSysPage(duint8* decompSec, duint32 decompSize)
{
	...
	{
		DRW_DBGH((unsigned char)tmpCompSec[i]);
		if (j == 7)
		{
			DRW_DBG("\n"); j = 0;
		}
...
{
	DRW_DBG("\nparseDataPage\n ");
	objData = new duint8[si.pageCount * si.maxSize + 0x1000];
	...
	for (auto it = si.pages.begin(); it != si.pages.end(); ++it)
	{
		...
		//get compresed data
		duint8* cData = new duint8[pi.cSize + 0x1000];
...
{
	//DRW_LEVEL(DRW_dbg::LEVEL::FILE, "18_file_header.log");
	DRW_DBG("dwgReader18::readFileHeader s\n");

	// encrypted data start at 0x80
	DRW_DBG("locate encrypted data at 0x80 \n");
	if (!fileBuf->setPosition(0x80))
	{
		DRW_DBG("locate encrypted data at 0x80 failed \n");
		return false;
	}
	//DRW_DBG("generate magic number 18\n");
	//genMagicNumber();
	...
	{
		duint8 ch = fileBuf->getRawChar8(); DRW_DBGH(ch);
		if (j == 15)
		{
			j = 0; DRW_DBG("\n");
		}
	...
	DRW_DBG("\nFile ID string (AcFssFcAJMB)= "); DRW_DBG(name);
	//ID string + NULL = 12
	buff.setPosition(12);
	...
	duint32 lastSecPageId = buff.getRawLong32();
	DRW_DBG("\nlast section page id"); DRW_DBG(lastSecPageId);
	// Last section page end address 64b=
	dint64 lastSecPageEndAddr = buff.getRawLong64();
	DRW_DBG("\nlast section page end address="); DRW_DBG(lastSecPageEndAddr);
	// Start of second header data address 64b=
	dint64 secondHeaderAdd = buff.getRawLong64();
	DRW_DBG("\nstart of second header data address="); DRW_DBG(secondHeaderAdd);
	// Gap amount=
	dint32 gapAmount = buff.getRawLong32();
	DRW_DBG("\ngap amount="); DRW_DBG(gapAmount);
	// Section page amount=
	duint32 pageAmount = buff.getRawLong32();
	DRW_DBG("\nsection page amount="); DRW_DBG(pageAmount);
	dint32 fixedVal;
	// 0x20
	fixedVal = buff.getRawLong32();
	DRW_DBG("\n0x20= "); DRW_DBG(fixedVal);
	// 0x80
	fixedVal = buff.getRawLong32();
	DRW_DBG("\n0x80= "); DRW_DBG(fixedVal);
	// 0x40
	fixedVal = buff.getRawLong32();
	DRW_DBG("\n0x40= "); DRW_DBG(fixedVal);

	// Section Page Map Id=
	dint32 secPageMapId = buff.getRawLong32();
	DRW_DBG("\nsection page map id="); DRW_DBG(secPageMapId);
	// Section Page Map address 64b=
	duint64 secPageMapAddr = buff.getRawLong64() + 0x100;
	DRW_DBG("\nsection page map address="); DRW_DBG(secPageMapAddr);
	// Section Map Id
	duint32 secMapId = buff.getRawLong32();
	DRW_DBG("\nsection map id="); DRW_DBG(secMapId);
	// Section page array size=
	dint32 secPageArrSize = buff.getRawLong32();
	DRW_DBG("\nsection page array size="); DRW_DBG(secPageArrSize);
	// Gap array size=
	dint32 gapArrSize = buff.getRawLong32();
	DRW_DBG("\ngap array size="); DRW_DBG(gapArrSize);
	// verify CRC
	duint32 crc = buff.getRawLong32();
	DRW_DBG("\nCRC32= "); DRW_DBG(crc);
	...
	//duint8* tmpDecompSec = new duint8[(duint64) decompSize + 0x100];
	std::vector<duint8> tmpDecompSec(decompSize + 0x1000);
	...
	//check section Map id
	if (sectionPageMapTmp.size() + 1 != secMapId)
	{
		DRW_DBG("There might be not data section map\n");
	}

	DRW_DBG("There might be a data section map\n");
	auto mapIt = sectionPageMapTmp.find(secMapId);
	if (mapIt == sectionPageMapTmp.end())
	{
		DRW_DBG("No data section map ,error\n");
		return false;
	}
	...
	tmpDecompSec.resize(decompSize + 0x1000);
	parseSysPage(tmpDecompSec.data(), decompSize);
	...
	//LOG_FILE(file_header);

	for (unsigned int i = 0; i < numDescriptions; i++)
	{
		DwgSectionInfo secInfo;
		secInfo.size = buff3.getRawLong64(); DRW_DBG("\nSize of section= "); DRW_DBGH(secInfo.size);
		secInfo.pageCount = buff3.getRawLong32(); DRW_DBG("\nPage count= "); DRW_DBGH(secInfo.pageCount);
		secInfo.maxSize = buff3.getRawLong32();
		DRW_DBG("\nMax Decompressed Size= ");
		DRW_DBGH(secInfo.maxSize);
		DRW_DBG("\nunknown long= ");
		DRW_DBGH(buff3.getRawLong32());
		secInfo.compresed = buff3.getRawLong32(); DRW_DBG("\nis Compressed? 1:no, 2:yes= "); DRW_DBGH(secInfo.compresed);

		if (secInfo.compresed != 1 && secInfo.compresed != 2)
		{
			DRW_DBG("\ninvalid compressed value from sections\n");
			return false;
		}

		secInfo.id = buff3.getRawLong32(); DRW_DBG("\nSection Id= "); DRW_DBGH(secInfo.id);
		if (secInfo.id != 0 && secInfo.id != numDescriptions - i)
		{
			DRW_DBG("invalid section");
			return false;
		}
		secInfo.encrypted = buff3.getRawLong32(); DRW_DBG("\nEncrypted= "); DRW_DBGH(secInfo.encrypted);
		duint8 nameCStr[64];
		buff3.getBytes(nameCStr, 64);
		secInfo.name = reinterpret_cast<char*>(nameCStr); DRW_DBG("\nSection std::Name= "); DRW_DBG(secInfo.name.c_str()); DRW_DBG("\n");
		if (SecEnum::DWGSection::UNKNOWNS == SecEnum::getEnum(secInfo.name))
		{
			DRW_DBG("\nWrong section read,error\n");
			return false;
		}
		for (unsigned int i = 0; i < secInfo.pageCount; i++)
		{
			duint32 pageNum = buff3.getRawLong32();
			DwgPageInfo pageInfo = sectionPageMapTmp[pageNum]; //get a copy
			DRW_DBG(" reading pag num = "); DRW_DBGH(pageNum);
			pageInfo.dataSize = buff3.getRawLong32();
			pageInfo.startOffset = buff3.getRawLong64();
			secInfo.pages[pageNum] = pageInfo; //complete copy in secInfo
			DRW_DBG("\n    Page number= "); DRW_DBGH(secInfo.pages[pageNum].id);
			DRW_DBG("\n    size in file= "); DRW_DBGH(secInfo.pages[pageNum].size);
			DRW_DBG("\n    address in file= "); DRW_DBGH(secInfo.pages[pageNum].address);
			DRW_DBG("\n    Data size= "); DRW_DBGH(secInfo.pages[pageNum].dataSize);
			DRW_DBG("\n    Start offset= "); DRW_DBGH(secInfo.pages[pageNum].startOffset); DRW_DBG("\n");
		}
...
bool DwgReader18::readDwgClasses()
{
	...
	if (ret)
	{
		...
		DRW::checkSentinel(&dataBuf, SecEnum::DWGSection::CLASSES, true);

		duint32 size = dataBuf.getRawLong32();
		DRW_DBG("\ndata size in bytes ");
		DRW_DBG(size);
		if ((version > DRW::R2007 && maintenanceVersion > 3) || version == DRW::R2018)
		{ //2010+
		...
		DRW::checkSentinel(strBuf, SecEnum::DWGSection::CLASSES, false);
...
bool DwgReader18::readDwgBlocks(DRW_Interface& intfa)
{
	bool ret = true;
	dwgBuffer dataBuf(objData, uncompSize, &decoder);
...
bool DwgReader18::readDwgEntities(DRW_Interface& intfa)
{
	bool ret = true;
	dwgBuffer dataBuf(objData, uncompSize, &decoder);
...
bool DwgReader18::readDwgObjects(DRW_Interface& intfa)
{
	bool ret = true;
	dwgBuffer dataBuf(objData, uncompSize, &decoder);
